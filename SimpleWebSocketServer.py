...
class SimpleWebSocketServer(object):
    ...
    def serveforever(self):
        while True:
            #Mostramos los segundos que llevamos en cada momento para ver si funciona correctamente el envio
            print str(time.localtime().tm_sec)
            #Si el tiempo local en segundos es 0, entocnes ejecutamos el codigo para enviar los datos a los clientes.
            if (time.localtime().tm_sec == 0):
                #Realizamos un for para enviar a todos los clientes el mensaje horario.
                for conexiones in self.connections.itervalues():
                    try:
                        conexiones.handleTime()
